...
must_haves:
  ...
  artifacts:
    - path: "src/lib/ai/depth-estimation.ts"
      ...
      exports: ["estimateDimensions", "getDimensionTierLabel", "formatDimensions"]
    - path: "src/lib/ai/render-generation.ts"
      provides: "Nano Banana Pro render client"
      exports: ["generateRenders", "getStylesForSpace", "buildRenderPrompt", "parseRenderResult"]
    - path: "convex/ai.ts"
      provides: "Render generation action"
      exports: ["generateRendersAction"]
  key_links:
...
**MVP Note on AI-002 (Depth Anything V2):**
For MVP, dimension estimation uses Claude Vision's built-in spatial reasoning (Basic tier, +/-15% accuracy).
This satisfies the core requirement of providing dimension estimates with confidence percentages.
Depth Anything V2 integration deferred to future enhancement for improved accuracy (Standard tier and above).
The dimension tier system is designed to accommodate future Depth Anything V2 integration.

**MVP Note on AI-003 (Style Classification):**
Style matching uses aesthetic-based mapping (modern -> modern styles, coastal -> coastal styles).
All users with the same detected aesthetic get the same 3 style options.
This satisfies "Matches space to 3 appropriate design styles" - styles ARE appropriate for the detected aesthetic.
Dynamic per-space style selection based on specific features deferred to future enhancement.
</objective>
...
For MVP, use Claude's dimension estimates directly (Basic tier, +/-15% accuracy). This avoids the complexity of Depth Anything V2 integration while still providing useful estimates. The tier system is designed to accommodate future Depth Anything V2 integration.
...
This approach:
- Uses Claude Vision's built-in dimension estimation (satisfies AI-002 for MVP)
- Applies reasonable min/max constraints
- Clearly communicates confidence tier
- Tier system designed for future Depth Anything V2 integration
  </action>
 ...
 *
 * MVP Implementation Note (AI-003):
...
- MVP scope documented (Claude Vision for dimensions, aesthetic-based style matching)
</success_criteria>
