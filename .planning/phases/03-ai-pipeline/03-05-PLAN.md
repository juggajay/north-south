...
must_haves:
  ...
  key_links:
    - from: "src/lib/hooks/useProcessPhoto.ts"
      to: "convex/ai.ts"
      via: "useAction hook"
      pattern: "useAction.*api\\.ai\\.analyzeSpaceAction"
      note: "Depends on analyzeSpaceAction from Plan 03-02"
    - from: "src/lib/hooks/useProcessPhoto.ts"
      to: "convex/ai.ts"
      via: "useAction hook"
      pattern: "useAction.*api\\.ai\\.generateRendersAction"
      note: "Depends on generateRendersAction from Plan 03-03"
    - from: "src/lib/hooks/useProcessPhoto.ts"
      to: "src/lib/ai/claude-vision.ts"
      via: "helper imports"
      pattern: "import.*imageUriToBase64.*resizeImageForVision"
      note: "Consumes helpers created in Plan 03-02"
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 pb-safe-bottom bg-gradient-to-t from-black/80 to-transparent pt-20">
        ...
        <p className="text-white/40 text-xs text-center px-6 pb-4">
          Estimates from photo — site measure confirms final dimensions
        </p>
      </div>
    </motion.div>
  );
...
**IMPORTANT: This task depends on Convex actions from Plans 03-02 and 03-03:**
- `api.ai.analyzeSpaceAction` (from Plan 03-02)
- `api.ai.generateRendersAction` (from Plan 03-03)

Before implementing, verify these actions exist in convex/ai.ts and are properly exported.
...
export function useProcessPhoto() {
  ...
  // Convex actions - these must exist from Plans 03-02 and 03-03
  ...
  const mutation = useMutation({
    mutationFn: async (imageUri: string): Promise<PipelineResult> => {
      ...
      // Step 1: Convert and resize image (using helpers from Plan 03-02)
      const base64Raw = await imageUriToBase64(imageUri);
      const imageBase64 = await resizeImageForVision(base64Raw);

      // Step 2: Analyze space with Claude Vision (uses analyzeSpaceAction from Plan 03-02)
      ...
      // Step 3: Estimate dimensions (uses estimateDimensions from Plan 03-03)
      advanceStage('measuring');
      const dimensions: Dimensions = estimateDimensions(analysis, 'basic');

      // Step 4: Get style matches (uses getStylesForSpace from Plan 03-03)
      advanceStage('styling');
      const styles = getStylesForSpace(analysis);

      // Step 5: Generate renders (uses generateRendersAction from Plan 03-03)
...
- Consumes helpers and actions from Plans 03-02 and 03-03
  </action>
  <verify>
1. `npm run typecheck` passes
2. Verify Convex actions exist: grep for "analyzeSpaceAction" and "generateRendersAction" in convex/ai.ts
3. Verify helper imports work: grep for "imageUriToBase64" and "resizeImageForVision" in src/lib/ai/claude-vision.ts
  </verify>
  <done>Pipeline orchestration hook with progress tracking, verified dependencies</done>
</task>
...
- [ ] Convex actions (analyzeSpaceAction, generateRendersAction) called correctly
- [ ] Helper functions (imageUriToBase64, resizeImageForVision) imported and used
...
- All dependencies from Plans 03-02 and 03-03 verified working
</success_criteria>
