...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/configurator/ConfiguratorPage.tsx"
      to: "convex/react"
      via: "useConvexAuth"
...
<task type="auto">
  <name>Task 1: Create ConfiguratorPage integrating all components with auth enforcement</name>
  <files>
    src/components/configurator/ConfiguratorPage.tsx
    src/app/(tabs)/design/page.tsx
  </files>
...
**IMPORTANT per CONTEXT.md:** "Authentication: Login required to use configurator"

This is the SINGLE entry point for all configurator usage. The auth check here enforces
the login requirement. Users MUST be authenticated to access the configurator.
- If not authenticated: Shows sign-in prompt with redirect to /login
- If authenticated: Creates/loads design and shows full configurator
...
If NOT logged in: Shows sign-in prompt (auth enforcement working).
If logged in: Configurator loads with 3D viewport and wizard.
All components render without errors.
  </verify>
  <done>
ConfiguratorPage integrates all components with auth enforcement. Design tab entry point working. Login required per CONTEXT.md.
  </done>
</task>
...
- LOD for performance
- **Login required to access (auth enforcement in ConfiguratorPage)**
  </what-built>
  <how-to-verify>
**Pre-requisites:**
...
**0. Auth Enforcement (CONTEXT.md requirement)**
- [ ] Navigate to /design while logged OUT
- [ ] Should see "Sign in to use the configurator" prompt
- [ ] Clicking "Sign In" redirects to /login
- [ ] After login, configurator loads normally
...
- [ ] Drawers slide out on tap (Z-axis)
- [ ] Lift-up doors rotate upward on tap (X-axis)
...
- Auth enforcement working (login required per CONTEXT.md)
...
- Login required to access configurator (auth check in ConfiguratorPage)
