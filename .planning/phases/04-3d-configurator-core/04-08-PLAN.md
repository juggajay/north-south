...
export default function DesignPage() {
  ...
  const resetConfig = useCabinetStore((state) => state.resetConfig)
...
Navigate to /design/[nonexistent] - shows not found.
  ...
  <done>
Design page with auto-save and loading from Convex working.
  </done>
</task>

<task type="auto">
  <name>Task 3: Create shareable design page with deep link</name>
  <files>
    src/app/design/share/[id]/page.tsx
  </files>
  <action>
...
export default function SharePage() {
  ...
  const design = useQuery(api.designs.get, { id: designId })
  const duplicateDesign = useMutation(api.designs.duplicate) // Need to create this
  ...
  // Check if current user is owner (for edit access)
  // const isOwner = design.userId === currentUserId
...
export const duplicate = mutation({
  ...
  handler: async (ctx, { sourceId }) => {
    ...
    return await ctx.db.insert("designs", {
      userId: null, // Will be set when user saves
...
3D viewport is interactive (rotate, zoom).
  ...
  <done>
Shareable design page with deep link, preview, and duplicate functionality.
  </done>
</task>
...
- Design page loads saved config from Convex
- Changes auto-save (1 second debounce)
- Share page shows 3D preview
- Share link can be copied
- Authenticated users can duplicate shared designs
</verification>
