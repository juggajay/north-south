...
must_haves:
  truths:
    - "Team sees submissions sorted oldest-first (FIFO) to work through in order received"
    - "Team sees customer name, email, submission date, and selected options on each card"
    - "Team views complete configuration per submission (dimensions, modules, finishes)"
    - "Team can change status from New to In Review to Quoted via buttons"
    - "Team can type and save internal notes visible only to team members"
  artifacts:
...
export function SubmissionQueue() {
  ...
  // FIFO sorting: oldest submissions first (order by createdAt ASC)
  // This is the team's workflow - work through submissions in order received
  return (
...
**Note on FIFO sorting:**
The Convex query should order by `createdAt` ascending (oldest first). Verify the `listAll` query returns submissions in this order. If not, add `.order("asc")` to the query or sort client-side.
  </action>
  <verify>
...
- SubmissionQueue lists all submissions with real-time updates, oldest first
  </done>
</task>
...
4. If submissions exist:
   - Cards display with name, email, date, status
   - Submissions appear oldest-first (FIFO)
   - Can expand to see full config (dimensions, modules, finishes)
...
- Queue shows all submissions sorted oldest-first (FIFO workflow)
- Each submission card shows: name, email, date, status, options requested
- Expanding a card shows full configuration details
- Team can update status through the workflow (New -> In Review -> Quoted)
