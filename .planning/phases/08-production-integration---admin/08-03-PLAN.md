...
<task type="auto">
  <name>Task 1: Add listAll query to orders API</name>
  <files>convex/orders.ts</files>
  <action>
Add a query to list all orders for admin dashboard (similar to submissions.listAll):

Add this query to convex/orders.ts:
...
This follows the same pattern as submissions.listAll from Phase 06.
  </action>
  <verify>npx tsc --noEmit convex/orders.ts shows no errors</verify>
  <done>listAll query added to orders API</done>
</task>
