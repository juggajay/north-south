...
import { PanelCard, PanelNotFound } from "@/components/portal/PanelCard";
...
export default function PanelLookupPage({ params }: PageProps) {
  ...
  React.useEffect(() => {
    params.then((p) => {
      setQrCode(decodeURIComponent(p.qrCode));
    });
  }, [params]);

  // Query panel data - PUBLIC (no auth check)
  const panel = useQuery(
    api.panels.lookupByQrCode,
    qrCode ? { qrCode } : "skip"
  );

  // Loading state
  if (panel === undefined || qrCode === null) {
    return (
